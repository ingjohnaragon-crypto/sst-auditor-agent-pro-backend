...
class InvalidContractHookArguments(Exception):
    """
    A custom exception raised when invalid hook arguments are defined in a Contract.
    """
...
class InvalidSmartContractError(ContractException):
    """
    A custom exception raised when a smart contract is invalid. These errors should not be logged
    at error level.
    """
...
class IllegalPython(Exception):
    """
    A custom exception raised when running the ast contract parse.
...
class UnsupportedApiVersion(Exception):
    """
    A custom exception raised when running or parsing a contract that uses an unsupported
