    ...
    def _spec(cls, language_code=symbols.Languages.ENGLISH):
        ...
        spec = types_utils.ClassSpec(
            ...
            public_methods=[
                types_utils.MethodSpec(
                    ...
                    docstring="""
                    Returns the effective timestamp of the last successful
                ...
                """,
                    args=[
                        types_utils.ValueSpec(
                            name="event_type",
                            type="str",
                            docstring="The event type for which to fetch the last effective date.",
                        ),
                    ],
                    return_value=types_utils.ReturnValueSpec(
                        docstring="""
                            The `effective_date` of the last successful execution of
                            link:{baseURL}/reference/contracts/contracts_api_3xx/smart_contracts_api_reference3xx/hooks#scheduled_code[scheduled_code]
        ...
        public_methods = [
            types_utils.MethodSpec(
                name="get_postings",
                docstring="""
                     Returns a list of ``[./PostingInstruction]`` objects whose `value_datetime`
                     fall within the requested time window, together with their covering posting
                     instructions.
                     In the pre/post posting hooks, it also returns 1. the covering
                     `PostingInstructions` of the proposed/accepted `PostingInstructionBatch`
                     regardless of any postings hook requirement, and 2. if `include_proposed` is
                     set to `True`, the `PostingInstructions` of the proposed/accepted
                     `PostingInstructionBatch` will also be returned.
                ...
                args=[
                    types_utils.ValueSpec(
                        ...
                        type="bool",
                        docstring="""
                            If the current hook is pre/post-posting, the returned list will
                            include the `PostingInstructions` of the proposed
                            `PostingInstructionBatch` only if this is True, regardless of any
                            postings hook requirement.
                        """,
                    )
                ],
                return_value=types_utils.ReturnValueSpec(
            ...
            types_utils.MethodSpec(
                ...
                args=[
                    types_utils.ValueSpec(
                        ...
                        type="bool",
                        docstring="""
                            If the current hook is pre/post-posting, the returned
                            `ClientTransactions` will include the `PostingInstructions` of the
                            proposed `PostingInstructionBatch` only if this is True, regardless of
                            any postings hook requirement.
                        """,
                    )
                ],
                return_value=types_utils.ReturnValueSpec(
            ...
            types_utils.MethodSpec(
                ...
                docstring="""
                    Gets a list of "posting batches" objects whose `value_datetime` fall within the
                    requested time window, together with their covering posting batches.
                    In the pre/post posting hooks, it also returns 1. the covering
                    `PostingInstructionBatches` of the proposed/accepted `PostingInstructionBatch`
                    regardless of any postings hook requirement, and 2. if `include_proposed` is
                    set to `True`, the proposed/accepted `PostingInstructionBatch` will also be
                    returned.
                """,
                args=[
                    types_utils.ValueSpec(
                        ...
                        docstring="""
                            If the current hook is pre/post-posting, the returned list will
                            include the proposed `PostingInstructionBatch` only if this is True,
                            regardless of any postings hook requirement.
                        """,
                    )
                ],
                return_value=types_utils.ReturnValueSpec(
            ...
            types_utils.MethodSpec(
                ...
                args=[
                    ...
                    types_utils.ValueSpec(
                        name="new_schedule",
                        type="Dict[str, str]",
                        docstring="The new schedule. Cannot be an empty dictionary",
                    ),
                ],
                examples=[
                    types_utils.Example(
                        title="A simple schedule amendment to 23:00 on the 28th of each month",
                        code="vault.amend_schedule(event_type='EVENT_NAME', new_schedule={'day':'28', 'hour': '23'})",
                    )
                ],
            ),
            types_utils.MethodSpec(
