    ...
    def _spec(cls, language_code=symbols.Languages.ENGLISH):
        ...
        spec.public_methods["get_postings"] = types_utils.MethodSpec(
            ...
            args=[
                ...
                types_utils.ValueSpec(
                    ...
                    type="Optional[bool]",
                    docstring="""
                        If the current hook is pre/post-posting and the `fetcher_id` is not
                        populated, the returned list will include the `PostingInstructions` of the
                        proposed `PostingInstructionBatch` only if this is True, regardless of any
                        postings requirements using the `requires` or `fetch_account_data` decorators.
                        This argument defaults to True, unless the `fetcher_id`
        ...
        spec.public_methods["get_posting_batches"] = types_utils.MethodSpec(
            ...
            docstring="""
                ...
                regardless of any postings hook requirement, and 2. if `include_proposed` is
                set to `True`, the proposed/accepted `PostingInstructionBatch` will also be returned.
                The default ordering of the list is by `value_timestamp`; you can order/filter
                further using the sorted builtin and other builtin mechanisms.
            """,
            args=[
                ...
                types_utils.ValueSpec(
                    ...
                    docstring="""
                        If the current hook is pre/post-posting and the `fetcher_id` is not
                        populated, the returned list will include the proposed
        ...
        spec.public_methods["get_client_transactions"] = types_utils.MethodSpec(
            ...
            args=[
                ...
                types_utils.ValueSpec(
                    ...
                    docstring="""
                        If the current hook is pre/post-posting and the `fetcher_id` is not
                        populated, the returned `ClientTransactions` will include the
                        `PostingInstructions` from the proposed `PostingInstructionBatch` only if
                        this is True, regardless of any postings requirements using the `requires`
                        or `fetch_account_data` decorators.
                        This argument defaults to True, unless the `fetcher_id`
                        argument is populated, in which case setting the argument to True
                        will raise an `InvalidSmartContractError`.
                    """,
